...
def send_weather_history_by_date(
    ...
    *,
    ...
    try_delete_message(ctx, message.chat.id, date_prompt_msg_id)
    session_store.history_drafts.pop(user_id, None)
    session_store.user_states.pop(user_id, None)
...
def send_history_monthly_report(
    ...
    *,
    ...
    if mode == "monthly_year":
        ...
        result = get_monthly_history_for_month(float(lat), float(lon), city_label, year, month)
        ...
        result = get_monthly_climate_normals(float(lat), float(lon), city_label, month)
        try_delete_message(ctx, message.chat.id, wait_msg_id)
    else:
        ctx.bot.send_message(
            message.chat.id,
    ...
    if mode == "monthly_year" and prompt_msg_id:
        try_delete_message(ctx, message.chat.id, prompt_msg_id)
    ...
    formatter = getattr(ctx, "format_history_monthly_climate_response", format_history_monthly_climate_response)
    ctx.bot.send_message(
        message.chat.id,
        formatter(
            city_label,
