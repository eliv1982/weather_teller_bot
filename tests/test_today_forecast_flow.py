...
from flows import send_today_forecast_by_coordinates
from forecast_service import get_today_forecast_day, get_tomorrow_forecast_day, group_forecast_by_day
...
def test_send_today_forecast_stores_cache_and_renders_today_day_directly():
    forecast_items = [
    ...
    ctx = SimpleNamespace(
        ...
        get_today_forecast_day=lambda grouped: get_today_forecast_day(grouped, today=date(2026, 5, 2)),
        ...
        format_today_forecast_response=lambda city, day, items, is_remaining_day=False: formatter_calls.append(
            (city, day, len(items), is_remaining_day)
        ) or f"today {day} {city} {len(items)} {is_remaining_day}",
        build_ai_action_keyboard=lambda text, callback_data: {"text": text, "callback_data": callback_data},
    )
    session_store = SessionStore()
    session_store.user_states[7] = "waiting_today_forecast_city"

    result = send_today_forecast_by_coordinates(
        _message(),
        7,
        55.75,
        37.61,
    ...
    assert formatter_calls == [("Москва", "02.05", 2, True)]
    assert [message["text"] for message in bot.messages] == [
        "Прогноз на сегодня готов.",
        "today 02.05 Москва 2 True",
        "✨ Хочешь короткое пояснение прогноза?",
    ]
    assert bot.messages[-1]["reply_markup"] == {
        "text": "✨ Короткое пояснение прогноза",
        "callback_data": "ai_today_forecast_day:02.05",
    }


def test_send_today_forecast_handles_missing_today_without_crashing():
    bot = _FakeBot()
    ctx = SimpleNamespace(
        bot=bot,
        logger=SimpleNamespace(warning=lambda *args, **kwargs: None),
        get_forecast_5d3h=lambda lat, lon: [
            {"dt_txt": "2026-05-03 12:00:00", "main": {"temp": 10}, "weather": [{"description": "ясно"}]},
        ],
        group_forecast_by_day=group_forecast_by_day,
        get_today_forecast_day=lambda grouped: get_today_forecast_day(grouped, today=date(2026, 5, 2)),
        get_location_by_coordinates=lambda lat, lon: None,
        ...
        main_menu=lambda: "main-menu",
        format_today_forecast_response=lambda city, day, items, is_remaining_day=False: "unused",
    ...
    assert bot.messages[-1]["text"] == (
        "Не нашла прогноз на сегодня в ответе погодного сервиса. Попробуй открыть прогноз на 5 дней."
    )
